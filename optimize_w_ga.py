...
def mutate_params(gene, num_runs):
    ...
    for i in range(1,4):
        if random.random() < mut_prob:
            gene[i] += 2*(random.random()-0.5)*mut_mag*20
...
def range_coerce(x, upper, lower):
    if x> upper:
        return upper
    elif x < lower:
        return lower
...
def weighted_draw(num_runs):
    ...
    k = random.random()
    if k<.6:
        return random.randint(0,quartile)
    if k<.9:
        return random.randint(quartile,half)
    return random.randint(half,num_runs-1)
    ...
    #array([('Galahad', 1.7, 38), ('Arthur', 1.8, 41), ('Lancelot', 1.8999999999999999, 38)], dtype=[('name', '|S10'), ('height', '<f8'), ('age', '<i4')])    
